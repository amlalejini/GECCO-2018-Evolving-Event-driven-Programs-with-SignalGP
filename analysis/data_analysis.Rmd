...
Regerated figures for dissertation chapter.
...
get_problem <- function(treatment_id) {
  # print(grep("_ENV2_", treatment_id))
  if (grepl("_ENV2_", treatment_id, fixed=TRUE)) {
    return("Two-state Environment")
  } else if (grepl("_ENV4_", treatment_id, fixed=TRUE)) {
    return("Four-state Environment")
  } else if (grepl("_ENV8_", treatment_id, fixed=TRUE)) {
    return("Eight-state Environment")
  } else if (grepl("_ENV16_", treatment_id, fixed=TRUE)) {
    return("Sixteen-state Environment")
  } else {
    return("unknown")
  }
}

get_condition <- function(treatment_id) {
  # 'Imperative', "Event-driven", 'Combined'
  if (grepl("ED0_AS1", treatment_id, fixed=TRUE)) {
    return("Imperative")
  } else if (grepl("ED1_AS0", treatment_id, fixed=TRUE)) {
    return("Event-driven")
  } else if (grepl("ED1_AS1", treatment_id, fixed=TRUE)) {
    return("Combined")
  } else {
      return("unknown")
  }
...
# Label problems from treatment id
chgenv_data$problem <- mapply(
  get_problem,
  chgenv_data$treatment
)
chgenv_data$problem <- factor(
  chgenv_data$problem,
  levels=c(
    "Two-state Environment",
    "Four-state Environment",
...
# Label condition from treatment id
chgenv_data$condition <- mapply(
  get_condition,
  chgenv_data$treatment
)
chgenv_data$condition <- factor(
  chgenv_data$condition,
  levels=c(
    "Imperative",
    "Event-driven",
    "Combined"
...
```{r}
ggplot(
    filter(chgenv_data, analysis=="fdom"),
    aes(x=condition, y=fitness, fill=condition)
  ) +
  geom_flat_violin(
  ...
  ggsave("chg-env-fitness.pdf", width=12, height=6)
```

#### Changing environment knockout performance

```{r}
ggplot(
    filter(chgenv_data, condition=="Combined"),
    aes(x=analysis, y=fitness, fill=analysis)
  ) +
  geom_flat_violin(
    position = position_nudge(x = .2, y = 0),
    alpha = .8
  ...
  scale_x_discrete(
    name="Analysis",
    breaks=c("fdom", "no_sensors", "no_signals"),
    limits=c("fdom", "no_sensors", "no_signals"),
    labels=c("Base", "No Sensors", "No Events")
  ) +
  scale_y_continuous(
    name="Fitness",
    limits=c(-1, 260),
  ...
  ggsave("chg-env-combined-strategy.pdf", width=12, height=6)
```
...
election_data_overtime$treatment <- factor(
  election_data_overtime$treatment,
  levels=c("Imperative_MsgForking", "Imperative_MsgNonForking", "EventDriven_MsgForking")
)
```
...
election_final_performance_plot <- ggplot(
    election_data,
    aes(x=treatment, y=max_fitness, fill=treatment)
  ) +
  geom_flat_violin(
    position = position_nudge(x = .2, y = 0),
    alpha = .8
  ...
  scale_x_discrete(
    name="Condition",
    breaks=c("Imperative_MsgForking", "Imperative_MsgNonForking", "EventDriven_MsgForking"),
    limits=c("Imperative_MsgForking", "Imperative_MsgNonForking", "EventDriven_MsgForking"),
    labels=c("Imperative\n(fork-on-retrieve)", "Imperative\n(copy-on-retrieve)", "Event-driven")
  ) +
  scale_y_continuous(
    name="Fitness",
    limits=c(-10, 7000),
...
election_final_performance_plot
```
...
# x=update
# y=max_fitness
election_ot_plot <- ggplot(
    election_data_overtime,
    aes(x=update, y=max_fitness, fill=treatment, color=treatment)
  ) +
  stat_summary(
    fun.y="mean",
    geom="line"
...
grid <- plot_grid(
  election_final_performance_plot,
  election_ot_plot + theme(axis.ticks.y=element_blank(),axis.text.y=element_blank(),axis.title.y=element_blank()),
  nrow=1,
  align="h",
  axis="l"
)
